...
**FTR-1 COMPLETE / INDEPENDENTLY VERIFIED / ACCEPTED**, recorded at **FE-EV-039**. This bounded, unscored governance amendment was independently accepted through the supplied FTR-1V **59 PASS · 1 FAIL, Decision B** → FTR-1F1 **single event-index traceability correction** → FTR-1V2 **33 PASS · 0 FAIL, Decision A** chain; prior architecture acceptance did not substitute for that review. FTR-1R records the supplied independent acceptance and performs no new verification. **FTR-2: COMPLETE / INDEPENDENTLY VERIFIED / ACCEPTED (FE-EV-041), from supplied FTR-2V 40 PASS / 0 FAIL, Decision A; independent tests 30 PASS / 0 FAIL / 0 ERROR. LR-1: COMPLETE / INDEPENDENTLY VERIFIED / CLOSED (FE-EV-043), recording supplied LR-1V 36 PASS / 0 FAIL, Decision A. LR-2: NEXT LEARNING-READY REMEDIATION BATCH / NOT STARTED. FE-1/FE-2/FE-3: NOT STARTED. Learning-Ready RC: NOT YET ASSESSED.** LR-1R records exactly FE-L1-002 and FE-L1-007 CLOSED / VERIFIED and approved controlled baseline evolution on `feature/lr-1-part-i-readiness`, at HEAD `5028e22c2f07083b8faa24e7df797d8286a97033`. The incoming 17-file LR-1V package was rehashed before closure edits; the complete 19-file package remains unstaged, uncommitted and unpushed for fresh narrow LR-1CV. FE-EV-042 and all prior event history remain unchanged. The final identities below include the Part-I README closure wording. HEAD/index still contain the pre-LR-1 controlled bytes, so both manifest checks correctly fail those layers before checkpoint while the working-tree identities match. The learning-ready and first-edition profiles retain unimplemented later-baseline checks; no Learning-Ready or edition gate approval follows. Integration and LR-2 require separate authorisation after closure verification/checkpoint; v0.16.0 remains ACTIVE / UNRELEASED / NOT RELEASE-READY and v0.15.0 remains latest stable locally.

Findings §8 now records the supplied LR-1V evidence and LR-1R closure; outstanding allocation is 0/3/6/5/4 = 18. The original LR-1 two-finding allocation, LR-BLOCKER disposition, owner Decision A and unscored authorship remain historical evidence. Only the two supplied verified closures change finding lifecycle; severity, class, ownership, provenance, Decision B and quantitative specialist controls remain binding. LR-1R records the independent result without claiming to perform LR-1V or LR-1CV.
...
[Findings §8](FIRST_EDITION_FINDINGS.md#8-learning-ready-fast-track-execution-overlay) is the canonical current allocation of the 18 outstanding findings. It overlays the historical H4–H7 architecture without deleting its provenance, closure criteria or specialist controls. H1/H2/H3 remain independently verified and closed. LR-1 is complete, independently verified and closed on supplied LR-1V evidence; LR-1R closure preparation awaits narrow independent LR-1CV. LR-2 is the next learning-ready remediation batch and has not started; FE-1/FE-2/FE-3 and the remaining historical H4–H7 scope have not started.
...
**Identity method.** Each manifest is SHA-256 of UTF-8, newline-terminated, path-sorted `path:Git-blob-SHA` records. HEAD and index blobs are read from Git; working-tree bytes are hashed with read-only `git hash-object --no-filters --stdin`. The working inventory also includes on-disk untracked/ignored chapter and Part-README candidates. All three layers are compared to the post-LR-1 closure-candidate references under the supplied baseline-evolution approval, recorded at FE-EV-043: 137 chapters, `cd0eea1a66d0bd9f5ea6ae8b27d782ec00ab1722d0d9af14b7ec828bb15d3a52`; twelve Part READMEs, `11437d6981fa1c28555ef4ef42b2db9cb369df658d43cdd09f671d47d3d5f0d6`. LR-1V approved the pre-closure chapter digest unchanged and the pre-closure Part-README digest `8eb51ec265fd18530e0b50696a15a80ff0db3ab1a58e55ebaec9d92b28f79b31`; the final Part-README digest includes the LR-1R closure paragraph and requires fresh narrow LR-1CV. Original FTR-2 references remain immutable in FE-EV-040/041. At the uncommitted LR-1R candidate, HEAD/index retain those old identities, so the manifest checks return FAIL for those layers while the working tree matches; no layer is exempted. This preserves the accepted index algorithm while also detecting unstaged mutation. Controlled symlinks and unresolved index conflicts are rejected. These references are neither a Learning-Ready freeze nor the Phase K baseline.

**Record parsing.** Canonical event headings and the Event index table are checked for valid IDs, uniqueness, equal sets and a continuous ascending body sequence from 001; the maximum is derived, never fixed at 039. Each Event ID field must agree with its heading. Finding records are the `### FE-…` sections, not systemic roots, historical summaries or allocation references. Their Status/Verification fields must be valid and unique; CLOSED requires VERIFIED, and OPEN cannot be VERIFIED. The post-LR-1 snapshot requires 29 records (18 OPEN / NOT VERIFIED; eleven CLOSED / VERIFIED) and the §8.2 outstanding allocation requires LR-1/LR-2/FE-1/FE-2/FE-3 counts 0/3/6/5/4 = 18, with every open ID allocated exactly once. An empty Finding IDs cell represents zero outstanding entries; empty members within a nonempty list, unknown IDs, closed IDs, missing open IDs and duplicate allocation still fail. Historical LR-1 allocation remains recorded outside the outstanding table. These are structural checks, not evidence that closure or allocation is substantively correct.

**Independent-review use and limitations.** Freeze the exact dirty package and authorised path inventory, inspect this implementation independently, rerun the fixtures and applicable profiles, and re-derive primary evidence required by §11. Author tests are unscored and close nothing. The current constants are pinned to the post-LR-1 closure-candidate identities and census under approved baseline evolution; fresh narrow LR-1CV must verify this closure delta before checkpoint. Later authorised manuscript correction or finding disposition will again fail the references until separately authorised baseline evolution is implemented and independently reviewed. It does not validate Markdown generally, historical event immutability across commits, committed batch changes, ignored files outside the controlled populations, submodule contents, mode-only manifest changes, technical truth, source support, learning suitability, owner approvals, systemic closure, quantitative semantics or complete final-gate coverage. It does not take an atomic repository snapshot; run on a stable worktree. No baseline acceptance can be inferred from matching hashes or package names.
