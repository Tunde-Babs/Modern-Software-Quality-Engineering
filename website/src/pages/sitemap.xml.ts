...
export function GET() {
  const routes = ['/', '/license/', '/handbook/', '/resources/', '/paths/', ...learningPaths.map(path => path.route), ...resourceCollections.map(collection => collection.route), ...[...registry.values()].map(entry => entry.route)].sort();
