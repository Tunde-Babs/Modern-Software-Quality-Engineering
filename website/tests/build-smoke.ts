...
for (const asset of ['pagefind/pagefind.js', 'pagefind/wasm.en.pagefind', 'search.js', 'favicon.svg', 'favicon.ico', '_headers']) assert.ok(files.includes(resolve(dist, asset)), `Missing public asset ${asset}`);
...
for (const [route, $] of urls) {
  ...
  assert.equal($('meta[name="robots"]').attr('content'), production && !['/search/', '/404.html'].includes(route) ? 'index, follow' : 'noindex, nofollow');
...
writeFileSync('artifacts/build-census.json', JSON.stringify({ mode: production ? 'production' : 'preview', indexedPages: searchEntry.languages.en.page_count, jsBytes: sizes.filter(file => file.path.endsWith('.js')).reduce((n, file) => n + file.bytes, 0), cssBytes: sizes.filter(file => file.path.endsWith('.css')).reduce((n, file) => n + file.bytes, 0), pagefindBytes: sizes.filter(file => file.path.startsWith('pagefind/')).reduce((n, file) => n + file.bytes, 0), pages: pages.length, chapters: chapterSources.length, parts: partSources.length, resources: resourceEntries.length + resourceCollections.length, resourceDocuments: resourceEntries.length, resourceCollections: resourceCollections.length, learningPaths: learningPaths.length, sitemapUrls: sitemap('loc').length, internalLinks, brokenDestinations: 0, distBytes: sizes.reduce((n, file) => n + file.bytes, 0), largestPages: sizes.filter(file => file.path.endsWith('.html')).slice(0, 5), assets: sizes.filter(file => !file.path.endsWith('.html')) }, null, 2) + '\n');
