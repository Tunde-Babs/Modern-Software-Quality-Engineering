...
Part XI — System Design & Architecture has passed its **Final Quality Gate at 96/100 (verdict A)** with controlled manuscript baseline `7067ebb54cba199a9215363188171d2e4966ed15`. **v0.14.0 is the current unreleased Release Candidate on `release/v0.14.0`**, created from the merged `develop` state after feature-to-`develop` integration. Chapters 1–12 remain `Status: Draft`. **No annotated tag and no GitHub Release exist; v0.13.0 remains the latest stable release.**
...
- Open the reviewed release pull request from `release/v0.14.0` to `main`.
