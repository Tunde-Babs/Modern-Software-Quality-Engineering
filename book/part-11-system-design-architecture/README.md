...
**Released as v0.14.0 — System Design & Architecture Complete on 2026-08-14. Final Part XI Quality Gate: 96/100 — verdict A. Controlled manuscript baseline `7067ebb54cba199a9215363188171d2e4966ed15`.** The release branch was merged to `main`, annotated tag `v0.14.0` was created, and the GitHub Release was published. All twelve chapters carry `Status: Draft` under manuscript-status governance. The Final Gate recorded **no P0, no P1, and no P2 finding**, with six non-blocking P3 items. **The Part XI release lifecycle is complete.** Part XII — Engineering Leadership & Career Growth has since been released as **v0.15.0**, which is now the latest stable release. No Part XI laboratory, diagram, ADR example, simulator, dataset, companion implementation, case-study file, website asset, CI/CD configuration, or infrastructure has been created.
...
Part XI — System Design & Architecture is released as **v0.14.0**, superseding Part X — Performance & Security Engineering at **v0.13.0**. Part XII — Engineering Leadership & Career Growth has since completed its own lifecycle and is released as **v0.15.0**, which is the latest stable release.
...
Part XI may advance from architecture planning to Pass 1 only after an independent architecture review accepts this plan or records approved corrections. **That precondition was met** and Pass 1 is complete. A future manuscript release may be considered only when:
...
- **Release state:** **v0.14.0 — System Design & Architecture Complete** is **released** (2026-08-14). Part XII — Engineering Leadership & Career Growth has since completed its own lifecycle and is released as **v0.15.0 — Engineering Leadership & Career Growth Complete** (2026-08-15), which is the latest stable release.
...
- **Release administration:** Complete. Feature-to-`develop` integration merged via pull request #35, `release/v0.14.0` was validated and merged to `main` via pull request #36, annotated tag `v0.14.0` was created, and the GitHub Release was published on 2026-08-14. **No further Part XI release administration is pending.**
- **Next authorized action:** None for Part XI — its lifecycle is closed. **Part XII — Engineering Leadership & Career Growth**, the handbook part that followed Part XI, has since completed its own lifecycle and is released as **v0.15.0**. All twelve handbook parts are now complete and released, and **`v0.16.0 — First Edition Review`** is the active, unreleased milestone. Phase F review execution is complete and Phase H targeted remediation is in progress; this does not make v0.16.0 release-ready. Do not create Part XI Pass 2 assets automatically. One control remains open and is deliberately carried forward rather than closed: ISO/IEC 25010:2023 and ISO/IEC/IEEE 42010:2022 are paywalled and returned HTTP 403 to every access attempt, so their characteristic and subcharacteristic placements were verified against consistent secondary references and the repository's already-gated Part III precedent — **not** against a purchased copy. That verification has not been upgraded and should be completed when a copy is available.
